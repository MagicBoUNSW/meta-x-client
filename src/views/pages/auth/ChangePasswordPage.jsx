...
import { changePassword } from "reducers/authentication";
import { useHistory } from "react-router-dom";
import { generateChangePasswordToken } from "../../../reducers/authentication";
...
const ResetPassword = () => {
  ...
  const generatePasswordToken = async () => {
    const result = await generateChangePasswordToken(email)
  ...
  const handleSubmit = async () => {
    setErrorMessage("");
    const result = await changePassword(email, oldPassword, password, resetToken);
    if (result.success) {
    ...
    <div className={classes.loginPage} style={{height:"fit-content"}}>
      ...
      >
        <Grid item xs={12} md={12} sm={12} lg={12}>
          <h1 className={classes.heading}>Change password successfully!</h1>
      ...
      >
        <Grid item xs={12} md={12} sm={12} lg={12}>
          <Grid container spacing={1}>
            ...
            <Grid item container direction="column" alignItems="flex-start">
              <h2 className={classes.title}>Current Password</h2>
              <InputField
                type="password"
                id="currentPassword"
                variant="outlined"
            ...
            <p><span onClick={generatePasswordToken} className={classes.sendTokenButton}>Send token</span></p>
